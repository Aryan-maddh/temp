...
from app.core.db_table_names import (
    CREDENTIAL_PLATFORM_COL,
    FORM_ANSWER_PLATFORM_COL,
...
class Credential(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "candidate_id", CREDENTIAL_PLATFORM_COL,
            name=f"uq_credential_candidate_{CREDENTIAL_PLATFORM_COL}",
        ),
    )
    ...
    # Normalized platform key: "workday", "greenhouse", "linkedin", "indeed", etc.
    # DB column name is configurable via CREDENTIAL_PLATFORM_COL env var
    # (old DB uses "domain", new DB uses "platform").
    platform: Mapped[str] = mapped_column(CREDENTIAL_PLATFORM_COL, Text, nullable=False)
    email: Mapped[str] = mapped_column(Text, nullable=False)
...
class FormAnswer(Base):
    ...
    # Normalized platform key: "workday", "greenhouse", "linkedin", etc. (NULL = global/cross-platform)
    # DB column name is configurable via FORM_ANSWER_PLATFORM_COL env var
    # (old DB uses "domain", new DB uses "platform").
    platform: Mapped[str | None] = mapped_column(FORM_ANSWER_PLATFORM_COL, Text)
