...
from backend.engine.classifier import PageKind, classify_page
from backend.models import Candidate, Credential
...
async def _login_rejected(page: object) -> bool:
    try:
        ...
        return "wrong email address or password" in text or "account might be locked" in text
    except Exception as e:
...
async def handle_login(
    ...
    *,
    ...
    # Give Workday a moment to process — some tenants take 2-4 seconds to
    # transition off the signup form. The previous defensive bail-out fired
    # before this transition completed; we now mirror Codex's working flow:
    # classify, persist credentials, and return whatever state we landed on
    # so the main step loop can drive the next page (verification, login,
    # or directly into the application form).
    ...
    if pending_signup_password and page_kind == "login":
        ...
        await _upsert_credential(db, None, candidate, domain, pending_signup_password)
        credential = await _find_credentials(db, candidate, domain)
        if credential is None:
        ...
        if not await _wait_for_login_fields(page, active_scope):
            from backend.engine.email_verifier import detect_verification_type, handle_email_verification
            ...
            if verification_type in {"otp", "magic_link", "account_exists", "verified"}:
                ...
                verification_result = await handle_email_verification(page, domain, email=candidate.email)
