...
from app.core.db import ROOT_DIR, SessionLocal
from app.engine import anti_ban
from app.engine.auth_handler import ManualLoginRequired, handle_login, _strong_password
from app.engine.classifier import classify_page
...
from app.engine.page_inspector import (
    decide_action,
...
from app.engine.platforms import action_override_for_page, handler_name_for_page, page_blocker, page_state_for_page
...
from app.engine.site_rules import domain_from_url, record_rule_failure, save_field_rule
from app.core.logs import write_run_log
from app.core.models import Application, Candidate, Credential, FormAnswer, Job, UnansweredQuestion
...
async def _saved_answers(db: AsyncSession, candidate: Candidate, page_url: str) -> dict[str, str]:
    ...
    platform = platform_key_for_url(page_url)
    ...
    for uq in answered_uq_rows:
        ...
        # Store using platform key so answers reuse across tenants of the same platform.
        uq_platform = platform_key_for_url(uq_domain or "") if uq_domain else None
        if not uq_label or not uq_answer:
            continue
        key = (str(candidate.id), uq_platform or "", uq_label)
        ...
        existing = await db.execute(
            ...
            .where(FormAnswer.platform.is_(None) if uq_platform is None else FormAnswer.platform == uq_platform)
    ...
    for answer in priority_rows:
        ...
        if label and value and label not in answers:
            answers[label] = value
        if label and value and platform and answer.platform == platform and label not in domain_answers:
...
def _candidate_dict(candidate: Candidate, saved_answers: dict[str, str] | None = None) -> dict[str, object]:
    ...
    if resume_path:
        ...
        if not path.is_absolute():
            # ROOT_DIR points to src/app — NOT the project root.
            # Resumes are stored at the top-level project resumes/ directory.
            # Try CWD first (correct when the server is started from the project
            # root via run_backend.py / run_backend_v2.py), then fall back to
            # the inferred project root derived from this file's location
            # (src/app/engine/browser.py → parents[3] = project root).
    ...
    _parsed = candidate.parsed_resume if isinstance(candidate.parsed_resume, dict) else {}
    return {
        "name": candidate.name,
        ...
        "work_experience": _parsed.get("work_experience") or [],
        "education": _parsed.get("education") or [],
    }
...
async def _workday_generic_auth_answers(
    ...
    _platform_key = platform_key_for_url(str(page_data.get("url") or ""))
    result = await db.execute(
        select(Credential).where(
            Credential.candidate_id == candidate.id,
            Credential.platform == _platform_key,
        )
    )
            ...
            try:
                ...
                if is_chip_picker:
                    ...
                    if not chip_added:
                        ...
                        await asyncio.sleep(0.5)
                    return None
                # ── Autocomplete/typeahead suggestion click ────────────────────────
                # For school / university / institution text fields a dropdown
                # suggestion list appears after typing.  Click the best-matching
                # item so the field receives a proper structured selection rather
                # than raw typed text (which sites often reject).
                # If no matching item is found, fall back to "Other" in the list.
                _is_autocomplete_label = any(
                    tok in label_text
                    for tok in ("school", "university", "institution", "college", "alma mater")
                )
                if _is_autocomplete_label:
                    _typed_value = str(action.get("value") or "").strip()
                    await asyncio.sleep(0.8)   # let the site populate suggestions
                    _suggestion_clicked = False
                    try:
                        _sugg_selectors = (
                            "ul[role='listbox'] li",
                            "[role='listbox'] [role='option']",
                            ...
                            ".suggestions li",
                            "[class*='suggestion']:not([aria-hidden='true'])",
                            "[class*='autocomplete-item']",
                            "[class*='dropdown-item']:not(.disabled)",
                        )
                        _combined = ", ".join(_sugg_selectors)
                        try:
                            await page.wait_for_selector(_combined, state="visible", timeout=2500)
                        except Exception:
                            pass
                        _suggestions = await page.evaluate(
                            """(combined) => {
                                const visible = (el) => {
                                    if (!el) return false;
                                    const r = el.getBoundingClientRect();
                        ...
                        if _suggestions:
                            _typed_lower = _typed_value.lower()
                            _best = None
                            for _s in _suggestions:
                                if _typed_lower in _s.get("text", "").lower():
                                    _best = _s
                                    break
                            # No match → fall back to "Other" option in the list
                            if _best is None:
                                for _s in _suggestions:
                                    if _s.get("text", "").strip().lower() in {"other", "other school", "other institution", "other university"}:
                                        _best = _s
                                        break
                            # Still nothing → try clicking any "Other" text in the dropdown
                            if _best is None:
                                _best = {"text": "Other"}
                            _best_text = _best.get("text", "")
                            _clicked = await page.evaluate(
                                """(args) => {
                                    const {combined, targetText} = args;
                                    const norm = t => (t || '').replace(/\\s+/g,' ').trim().toLowerCase();
                                    const want = norm(targetText);
                                    const els = Array.from(document.querySelectorAll(combined));
                                    const pick = els.find(el => norm(el.innerText || el.textContent || '') === want)
                                        || els.find(el => norm(el.innerText || el.textContent || '').includes(want));
                                    if (!pick) return false;
                                    pick.click();
                                    return true;
                                }""",
                                {"combined": _combined, "targetText": _best_text},
                            )
                            if _clicked:
                                await asyncio.sleep(0.4)
                                _suggestion_clicked = True
                                await log(
                                    application_id, "info",
                                    f"Step {step}: autocomplete suggestion clicked for [{label_text}]: [{_best_text}]",
                                    db,
                                )
                    except Exception as _ae:
                        logger.debug("autocomplete suggestion click failed for [%s]: %s", label_text, _ae)
                    if _suggestion_clicked:
                        return None
                await _dispatch_change(page, selector)
            except Exception as exc:
                await log(
...
def _ask_user_tkinter_sync(field_label: str, field_type: str, options: list[str]) -> str:
    ...
    if options:
        ...
        # Show a date hint. Only pre-fill today for signature/consent date fields,
        # NOT for work experience from/to dates (those should come from the profile).
        _label_lower = field_label.lower()
        _is_date = (
            field_type in ("date",)
            or any(tok in _label_lower for tok in ("date", "start date", "availability"))
        )
        _is_signature_date = _is_date and any(
            tok in _label_lower for tok in ("today", "signature", "consent", "sign date", "current date")
        )
        if _is_date:
...
async def _prompt_and_execute_unanswered(
    ...
    """
    from app.core.models import FormAnswer as _FA
    ...
    for action in actions:
        ...
        try:
            ...
            if _existing_row:
                ...
                db.add(_FA(candidate_id=candidate.id, platform=platform,
                           question_text=label, answer=answer))
            await db.commit()
        except Exception as fa_exc:
            logger.warning("save tkinter FormAnswer failed: %s", fa_exc)

        # ── Bug fix: mark the application_qa row as answered so the UI reflects it ──
        # Without this, recruiter_answer stays null and the question always appears
        # unanswered in the dashboard even though the user already provided an answer.
        try:
            _uq_result = await db.execute(
                _sa_select(UnansweredQuestion).where(
                    UnansweredQuestion.application_id == application_id,
                    UnansweredQuestion.candidate_id == candidate.id,
                    UnansweredQuestion.field_label == label,
                )
            )
            _uq_row = _uq_result.scalars().first()
            if _uq_row and not _uq_row.recruiter_answer:
                _uq_row.recruiter_answer = answer
                _uq_row.answered_at = datetime.now(timezone.utc)
                await db.commit()
        except Exception as uq_exc:
            logger.warning("update UnansweredQuestion from tkinter failed: %s", uq_exc)
...
def _norm_label(label: str) -> str:
    """Normalise a field label for tkinter_answered_labels membership tests.

    Collapses whitespace and lowercases so that slight formatting differences
    (e.g. page-inspector truncation vs. full DOM text) don't cause the same
    question to be prompted twice.
    """
    return " ".join(str(label or "").strip().lower().split())


def _label_already_tkinter_answered(label: str, answered: set[str]) -> bool:
    """Return True when *label* is in *answered* or is a prefix/suffix match.

    Workday sometimes returns very long question texts that get truncated at
    different lengths depending on which code path produced them (unanswered
    action vs. select/radio action).  A prefix match catches those cases so
    we never re-prompt the user for a question they already answered.
    """
    ...
    for al in answered:
        nal = _norm_label(al)
        if nal and (nl == nal or nl.startswith(nal) or nal.startswith(nl)):
            return True
    return False
...
async def run_page_loop(page: object, candidate: Candidate, db: AsyncSession, application_id: UUID) -> str:
    ...
    try:
        from app.engine.external_applier import run_vertex_vision_loop
    ...
    for step in range(1, MAX_STEPS + 1):
        ...
        if platform_blocker and platform_blocker.kind == "email_verification" and platform_handler != "workday":
            from app.engine.email_verifier import handle_email_verification
            _ev_domain = domain_from_url(str(page_data.get("url") or getattr(page, "url", "")))
            await log(
                application_id, "info",
                f"Step {step}: Email verification detected on {_ev_domain}; attempting Outlook auto-handle",
                db,
            )
            _ev_result = await handle_email_verification(page, _ev_domain, email=candidate.email)
            await log(application_id, "info", f"Step {step}: Email verification result: {_ev_result}", db)
            if _ev_result == "verified":
                await _wait_for_page_stability(page)
                continue
            application = await db.get(Application, application_id)
            if application is not None:
            ...
            await log(
                application_id, "warn",
                f"Step {step}: {platform_blocker.message} (Outlook auto-handle={_ev_result}; needs manual)",
                db,
            )
            return "needs_manual"
        domain = domain_from_url(str(page_data.get("url") or getattr(page, "url", "")))
        platform = platform_key_for_url(str(page_data.get("url") or getattr(page, "url", "")))
        ...
        if workday_auth_page:
            ...
            try:
                ...
                if page_kind == "login":
                    # ── Auto-retry before going to manual ──────────────────────────
                    # page_kind=="login" after handle_login() can happen when:
                    #   • Page was still transitioning (slow Workday tenant)
                    #   • Session expired mid-application → redirect to login
                    #   • Non-standard error text that _login_rejected didn't catch
                    # Retry up to 2 times with a short wait before asking the user.
                    _log_cb = lambda level, message: log(
                        application_id,
                        level if level != "warning" else "warn",
                        message,
                        db,
                    )
                    for _auth_retry in range(2):
                        # First just wait — the page may transition on its own
                        await asyncio.sleep(4)
                        _rekind = await classify_page(page)
                        if _rekind != "login":
                            page_kind = _rekind
                            await log(
                                application_id, "info",
                                f"Step {step}: auth page resolved naturally after wait → {page_kind}",
                                db,
                            )
                            break
                        await log(
                            application_id, "info",
                            f"Step {step}: auth retry {_auth_retry + 1}/2 — re-invoking login handler",
                            db,
                        )
                        try:
                            page_kind = await handle_login(
                                page, candidate, db, log_cb=_log_cb,
                            )
                            await log(
                                application_id, "info",
                                f"Step {step}: auth retry {_auth_retry + 1} returned {page_kind}",
                                db,
                            )
                            if page_kind != "login":
                                break
                        except ManualLoginRequired as _re:
                            await log(
                                application_id, "warn",
                                f"Step {step}: auth retry {_auth_retry + 1} — ManualLoginRequired: {_re}",
                                db,
                            )
                            # Don't return yet — allow the next retry iteration

                    if page_kind != "login":
                        # Retry succeeded — continue the main step loop normally
                        await _wait_for_page_stability(page)
                        await asyncio.sleep(2)
                        continue

                    # Still on login after all retries — fall back to manual wait
        ...
        if _required_unanswered_phase1:
            ...
            _to_prompt = [a for a in _required_unanswered_phase1
                          if not _label_already_tkinter_answered(str(a.get("label") or ""), tkinter_answered_labels)]
            _already_tkinter = [a for a in _required_unanswered_phase1
                                 if _label_already_tkinter_answered(str(a.get("label") or ""), tkinter_answered_labels)]
            if _to_prompt:
                await log(application_id, "warn",
