...
PLATFORM_TALEO = "taleo"
PLATFORM_SMARTRECRUITERS = "smartrecruiters"
PLATFORM_ASHBY = "ashby"
PLATFORM_BRASSRING = "brassring"
PLATFORM_SUCCESSFACTORS = "successfactors"
PLATFORM_APPLICANTSTACK = "applicantstack"
...
KNOWN_PLATFORMS: dict[str, str] = {
    ...
    PLATFORM_ASHBY: "Ashby",
    PLATFORM_BRASSRING: "BrassRing",
    PLATFORM_SUCCESSFACTORS: "SAP SuccessFactors",
    PLATFORM_APPLICANTSTACK: "ApplicantStack",
}
...
def platform_for_domain(domain_or_url: str) -> PlatformAdapter:
    ...
    if "linkedin.com" in hostname:
        return PlatformAdapter(PLATFORM_LINKEDIN, "")
    if "indeed.com" in hostname:
        return PlatformAdapter(PLATFORM_INDEED, "")
    if "lever.co" in hostname or "jobs.lever.co" in hostname:
        return PlatformAdapter(PLATFORM_LEVER, "")
    if "ziprecruiter.com" in hostname:
        return PlatformAdapter(PLATFORM_ZIPRECRUITER, "")
    if "icims.com" in hostname:
        return PlatformAdapter(PLATFORM_ICIMS, "")
    if "taleo.net" in hostname:
        return PlatformAdapter(PLATFORM_TALEO, "")
    if "smartrecruiters.com" in hostname:
        return PlatformAdapter(PLATFORM_SMARTRECRUITERS, "")
    if "ashbyhq.com" in hostname:
        return PlatformAdapter(PLATFORM_ASHBY, "")
    if "brassring.com" in hostname or "kenexa.com" in hostname:
        return PlatformAdapter(PLATFORM_BRASSRING, "")
    if "successfactors.com" in hostname or "successfactors.eu" in hostname:
        return PlatformAdapter(PLATFORM_SUCCESSFACTORS, "")
    if "applicantstack.com" in hostname:
        return PlatformAdapter(PLATFORM_APPLICANTSTACK, "")
    return PlatformAdapter(PLATFORM_GENERIC, "")


def platform_key_for_url(url: str) -> str:
    """Return normalized platform key for known platforms, raw hostname for generic sites.

    Stored in Credential.platform and FormAnswer.platform so all jobs on the
    same platform share one credential/answer set instead of one per tenant URL.
    """
    ...
    if "indeed.com" in hostname:
        return PLATFORM_INDEED
    if "lever.co" in hostname or "jobs.lever.co" in hostname:
        return PLATFORM_LEVER
    if "ziprecruiter.com" in hostname:
        return PLATFORM_ZIPRECRUITER
    if "icims.com" in hostname:
        return PLATFORM_ICIMS
    if "taleo.net" in hostname:
        return PLATFORM_TALEO
    if "smartrecruiters.com" in hostname:
        return PLATFORM_SMARTRECRUITERS
    if "ashbyhq.com" in hostname:
        return PLATFORM_ASHBY
    if "brassring.com" in hostname or "kenexa.com" in hostname:
        return PLATFORM_BRASSRING
    if "successfactors.com" in hostname or "successfactors.eu" in hostname:
        return PLATFORM_SUCCESSFACTORS
    if "applicantstack.com" in hostname:
        return PLATFORM_APPLICANTSTACK
    return hostname or PLATFORM_GENERIC
...
def workday_auth_page_detected(page_data: dict[str, object]) -> bool:
    ...
    # Some Workday tenants show a "Continue with Facebook / Google / LinkedIn /
    # Email" choice page after the user clicks Apply Manually.  Password fields
    # do NOT exist yet on this screen (they only appear after "Continue with
    # Email" is selected), so the normal has_password_field path misses it.
    # Detect it by looking for any social-provider button text.
    has_social_selector = any(
        phrase in text
        for phrase in (
