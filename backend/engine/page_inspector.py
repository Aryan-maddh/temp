...
from backend.engine.site_rules import domain_from_url, find_field_rule, normalize_label
from backend.engine.workday_intent import classify as workday_classify
...
async def decide_action(page_data: dict[str, Any], candidate: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    def synthetic_answer_for(label: str, field_type: str) -> str:
        ...
        if "degree" in text:
            return ""
        if "major" in text or "field of study" in text:
            return ""
        if "company" in text and "current" in text:
            return ""
    ...
    for field in page_data.get("fields", []):
        ...
        if field_type in ("select-one", "select-multiple"):
            ...
            if (
                ...
                and not saved_rule
                and best_answer_source not in {"extra", "saved_answer_intent_override", "saved", "saved_answer"}
