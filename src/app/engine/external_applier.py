...
from app.engine.page_inspector import inspect_page as _inspector_inspect_page
from app.engine.platform_adapters import is_third_party_apply_button, platform_key_for_url
from app.core.models import Application, ApplicationScreenshot, Candidate, FormAnswer, UnansweredQuestion
    ...
    try:
        ...
        if resume_path:
            ...
            if not path.is_absolute():
                ...
                # root), then fall back to the inferred project root from this file's
                # location (src/app/engine/external_applier.py → parents[3]).
...
async def _saved_answers(db: AsyncSession, candidate: Candidate, page_url: str) -> dict[str, str]:
    try:
        ...
        # NOTE: The Python attribute is always `platform` on the FormAnswer model,
        # regardless of whether the DB column is named "domain" (old jobapplier DB)
        # or "platform" (new jobapplier_v2 DB).  Using `row.domain` here raised
        # AttributeError and made this function silently return {} every time.
        priority_rows = sorted(
            rows,
            key=lambda row: 0 if platform and row.platform == platform else 1 if not row.platform else 2,
        )
        ...
        return bool(
            await page.evaluate(
                ...
                () => {
                  ...
                  // ── Social-login-selector screen ─────────────────────────────
                  // After clicking Apply Manually some Workday tenants show a screen
                  // with "Continue with Facebook / Google / LinkedIn / Email" buttons.
                  // There are NO password fields yet on this screen.  Detect it so the
                  // vision loop defers to the controlled auth handler which will click
                  // "Continue with Email" and then handle the resulting login form.
                  const hasSocialSelector = (
                    ...
                    text.includes('sign in with linkedin')
                  );
                  if (hasSocialSelector) return true;
...
async def _upload_resume(page: object, candidate_profile: dict[str, Any]) -> bool:
    ...
    # Fallback: if the candidate profile has no resume path (or the file is gone),
    # check saved_answers for an answer to a resume-related question that contains
    # a valid file path.  This handles the case where the user answered the
    # "Upload Resume" unanswered-question prompt with a local file path and that
    # answer was subsequently stored in FormAnswer (via the unanswered-questions
    # answer endpoint) — which is how users commonly provide their resume path
    # when candidate.resume_path was not set up-front.
    if not resume_path or not Path(resume_path).exists():
        saved: dict[str, str] = candidate_profile.get("saved_answers") or {}
        for sa_label, sa_value in saved.items():
            sa_label_norm = sa_label.lower().replace("_", " ").replace("-", " ")
            if re.search(r"\b(resume|cv|curriculum vitae|upload)\b", sa_label_norm) and sa_value:
                candidate_path = Path(sa_value.strip())
                if candidate_path.exists():
                    resume_path = str(candidate_path)
                    break
                # Also try resolving as relative to project root
                project_root = Path(__file__).resolve().parents[3]
                candidate_path_abs = project_root / sa_value.strip()
                if candidate_path_abs.exists():
                    resume_path = str(candidate_path_abs)
                    break
    ...
    *,
    ...
    for step in range(MAX_VISION_STEPS):
        ...
        if await _looks_like_email_verification_page(page) or result.page_type == "otp" or result.page_type in {"login", "signup"}:
            from app.engine.email_verifier import detect_verification_type, fetch_otp, handle_email_verification
...
async def otp_fetcher() -> str | None:
    from app.engine.email_verifier import fetch_otp
