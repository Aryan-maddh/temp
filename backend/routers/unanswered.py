...
from backend.engine.site_rules import save_field_rule
from backend.models import Application, FormAnswer, Job, UnansweredQuestion
from backend.schemas import UnansweredQuestionOut
...
def _dispatch_restarted(application_ids: list[uuid.UUID]) -> None:
    try:
        from backend.routers.applications import spawn_dispatch
    except Exception:
        return
...
async def batch_answer(
    ...
    for item in payload.answers:
        ...
        if payload.save_rule and q.field_label and q.candidate_id:
            ...
            if domain and q.field_label and q.field_type:
                save_field_rule(
                    domain=domain,
                    label=q.field_label,
...
async def answer_all_by_label(
    ...
    for q in matching:
        ...
        if payload.save_rule and q.candidate_id:
            ...
            if domain and q.field_label and field_type:
                save_field_rule(
                    domain=domain,
                    label=q.field_label,
